...
import { deletePrivateFile, savePrivateFile } from '../services/storage.js';
...
router.post('/:agreementId', upload.single('file'), async (req, res, next) => {
  let storageKey: string | undefined;
  try {
    const agreement = await canAccessAgreement(String(req.params.agreementId), req.user!.id);
    if (!agreement) return res.status(404).json({ error: 'Agreement not found' });
    if (!req.file) return res.status(400).json({ error: 'A PDF, JPEG, or PNG file is required' });
    const documentType = typeSchema.parse(req.body.documentType);
    const extension = allowed.get(req.file.mimetype)!;
    storageKey = await savePrivateFile({ buffer: req.file.buffer, extension, userId: req.user!.id, agreementId: agreement.id });
    const document = await prisma.document.create({ data: { userId: req.user!.id, agreementId: agreement.id, documentType, storageKey, status: 'PENDING' }, select: { id: true, documentType: true, status: true, createdAt: true } });
    return res.status(201).json({ data: document });
  } catch (error) {
    if (storageKey) await deletePrivateFile(storageKey).catch(() => undefined);
    if (error instanceof multer.MulterError) return res.status(400).json({ error: error.code === 'LIMIT_FILE_SIZE' ? 'File exceeds the 5 MB limit' : 'Invalid file upload' });
    return next(error);
  }
});
