...
router.post('/', requireRole('TENANT'), async (req, res, next) => {
  try {
    ...
    const enquiry = await prisma.enquiry.create({ data: { propertyId: property.id, tenantId: req.user!.id, ownerId: property.ownerId, message: input.message }, include: { property: true } });
...
router.put('/:id', requireRole('OWNER'), async (req, res, next) => {
  try {
    ...
    const existing = await prisma.enquiry.findFirst({ where: { id: String(req.params.id), ownerId: req.user!.id } });
