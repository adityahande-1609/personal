...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    response = await fetch(`${API_URL}${path}`, { ...init, credentials: 'include', headers: { 'Content-Type': 'application/json', ...(init?.headers || {}) } });
  } catch {
...
export async function getOwnerProperty(id: string) { return request<{ data: Property }>(`/owner/properties/${encodeURIComponent(id)}`); }
