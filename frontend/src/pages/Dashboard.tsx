...
export default function Dashboard({role}:{role:'OWNER'|'TENANT'}) {
  ...
  async function signOut() {
    try { await logout(); navigate('/'); } catch (err) { setError(err instanceof Error ? err.message : 'Unable to log out'); }
  }
  ...
  return <main className="page dashboard">
    ...
    <section className="stat-grid">
      <article className="stat"><strong>0</strong><span>{role === 'OWNER' ? 'Active listings' : 'Saved properties'}</span></article>
